...
namespace Utilities.Extensions
{
	...
	{
		public static float RandomBias(this float value, float range, bool useNegativeBias = true) => value + value * Random.Range(useNegativeBias ? -range : 0, range);

		public static float Round(this float value, int digits = 0) => (float)Math.Round(value, digits);

		public static bool IsApproximatelyEqual(this float value, float other, float tolerance = 0.0001f) => Math.Abs(value - other) < tolerance;
		...
		public static float Lerp(this float current, float target, float t) => Mathf.Lerp(current, target, Mathf.Clamp01(t));

		public static float LerpUnclamped(this float current, float target, float t) => Mathf.LerpUnclamped(current, target, t);
        ...
        public static float LerpAngle(this float current, float target, float time) => Mathf.LerpAngle(current, target, time);
        ...
        //
        //   maxDelta:
        public static float MoveTowardsAngle(this float current, float target, float time) => Mathf.MoveTowardsAngle(current, target, time);
        ...
        public static float SmoothStep(this float from, float to, float time) => Mathf.SmoothStep(from, to, time);
		...
		public static string ToMoneyText(this float money, float decimalSizeRate = 0.6f, string currency = "", float fontSize = 0f)
		{
			string text = !(money < 0f) ?  currency.IsNullOrEmpty() ? "$" : currency + ((float)Math.Round(money, 2)).ToString("0.00") : $"- {(currency.IsNullOrEmpty() ? "$": currency)}" + Mathf.Abs((float)Math.Round(money, 2)).ToString("0.00");
			text = text.Replace(',', '.');
			int num = text.IndexOf('.');
			if (num != -1 && fontSize > 0f)
			{
				string text2 = "<size=" + fontSize * decimalSizeRate + ">";
				text2 = text2.Replace(',', '.');
