...
namespace TUTORIAL_SYSTEM
{
    ...
    public class TutorialModule_QuestMarker : TutorialModule
    {
        ...
        private void UpdateMarkerPosition()
        {
            if (Target == null)
            {
                if (Application.isPlaying)
                    TutorialManager.Instance.DebugLog("Quest Marker (" + gameObject.name + ")  --> Target Null !", gameObject, TutorialManager.DebugType.Error);
                return;
            }
        ...
        public override IEnumerator ActivateModule() { yield return null; }
    }
}