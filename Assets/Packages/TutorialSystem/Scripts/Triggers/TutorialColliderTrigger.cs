            ...
            if (Collider2D)
            {
                ...
                if (YourCollider2D.gameObject.TryGetComponent<Rigidbody2D>(out var r) == false)
                {
                    if (r == null)
                    {
                        ...
                        TutorialManager.Instance.DebugLog("Added rigidbody2D to Target Collider(name:" + YourCollider2D.gameObject.name + ") 2D in stage '" + TutorialManager.Instance.Tutorials[tutorialIndex].Stages[stageIndex].TutorialStageName + "'", YourCollider2D.gameObject, TutorialManager.DebugType.Info);
                    }
                }
            ...
            else
            {
                ...
                if (YourCollider.gameObject.TryGetComponent<Rigidbody>(out var r) == false)
                {
                    if (r == null)
                    {
                        ...
                        TutorialManager.Instance.DebugLog("Added rigidbody to Target Collider(name:" + YourCollider.gameObject.name + ") in stage '" + TutorialManager.Instance.Tutorials[tutorialIndex].Stages[stageIndex].TutorialStageName + "'", YourCollider.gameObject, TutorialManager.DebugType.Info);
                    }
                }
